...
void initQueue()
{
    ...
    queue.s1->data = malloc(queue.s1->cap * sizeof(int));
    queue.s2->data = malloc(queue.s2->cap * sizeof(int));
...
void display()
{
    ...
    // Display should show queue from front → rear

    // Move s1 → s2 ONLY if s2 empty
    if (queue.s2->top == -1)
    {
        while (queue.s1->top != -1)
        {
...
int main()
{
    ...
    display();  // should show: 2 3 4 5

    dequeue();

    display();  // should show: 3 4 5
